...
export async function uploadFile(file: File, conversationId: string): Promise<string | null> {
  ...
  const { data, error } = await supabase.storage.from('attachments').createSignedUrl(uniqueName, 3600);
...
export async function scheduleMessage(
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) { console.error('Schedule message error:', error); return null; }
  return data;
}
